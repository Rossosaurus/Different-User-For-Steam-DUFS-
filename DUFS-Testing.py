...
#Wait so many seconds for Steam Home Streaming users to reconnect
time.sleep(int(config['SETTINGS']['client-gamewaittime']))

pathGame = config['PATHS']['gamepath']

if config.getboolean('SETTINGS', 'shsmode'):
    #Start game with GloSC
    if config['SETTINGS']['glosc-config']:
        with open('launchGloSC.bat', 'w+') as bat:
            bat.write('cd "C:/Program Files (x86)/GloSC/"\n"C:\Program Files (x86)\GloSC\SteamTarget.exe" "./targets/' + config['SETTINGS']['glosc-config'] + '"')
        process = subprocess.run('runas /savecred /user:' + config['SETTINGS']['newuser'] + ' "launchGloSC.bat"')
        
    #Else start game normally
    else:
